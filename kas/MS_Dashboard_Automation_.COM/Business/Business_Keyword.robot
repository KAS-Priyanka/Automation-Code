...
Select From Custom Dropdown For Business
    ...
    ...  xpath=//span[normalize-space()='${option_text}']
    ...
    IF    not ${found}
        Log To Console    Option not present: ${option_text}
    END
...
Download Profit And Loss Report
    ...
    Log To Console     PDF download 

    Wait Until Element Is Visible   ${CSV_BTN}   10s
    Wait Until Element Is Enabled   ${CSV_BTN}   10s
    Execute Javascript    document.evaluate("//span[normalize-space()='CSV']", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.click();


    Click Element    ${CSV_BTN}
    Log To Console     CSV download 
