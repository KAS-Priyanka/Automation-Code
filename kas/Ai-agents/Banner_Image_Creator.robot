...
3.Launch Banner Image Creator  
    [Documentation]    Validate AI agent card, launch Banner Image Creator
    Verify AI-agents Card  ${BANNER_IMAGE_CREATOR_CARD}  ${BANNER_IMAGE_CREATOR_TITLE}   ${BANNER_IMAGE_CREATOR_PRICE}  ${BANNER_IMAGE_CREATOR_AGENT_LAUNCH_BTN}
    Launch AI Agent   Banner Image Creator
  
4.Verify Banner Image Creator Page,Wallet Section And Template Download
    [Documentation]   Verify Banner Image Creator Page, Wallet Section And Template Download button
    Wait Until Page Contains   Banner Image Creator    5s   
    Log To Console    Banner Image Creator page loaded successfully
    Verify Wallet Section   ${WALLET_LABEL}     ${WALLET_VALUE}
...
6.Verify Valid ASIN Entry, Marketplace Selection, ASIN Removal and Product Upload
    ...
    Enter ASIN   B0BTD4S4XF  Valid ASIN entered successfully.
...
8.Add Product Description,Marketplace,Theme And Custom Scene
    ...
    Enter Product Description    ${PRODUCT_DESCRIPTION}    A stylish and durable bag for everyday use.
    Wait Until Page Contains  Logo    2s
    Upload Logo    ${LOGO_IMAGE}
    Select Banner Size  ${BANNER_SIZE_DROPDOWN}  Medium 2:1 (1500 × 750)
    Select Banner Text   Custom   Summer Sale    Get 50% off today
    Select Visual Style   Styles   Product Banner
    Wait Until Page Contains      Saving
    Log To Console    Saving...
    Select Visual Style   Styles   Product Banner
    Select Visual Style   Custom    Show the bag as the main focus with a premium gradient background and subtle shadows.
